...
public class StringsFoldingBuilderXml extends FoldingBuilderBase {
	@Override
	protected @Nullable List<FoldingDescriptor> collectFoldRegions(@NotNull ASTNode astNode, @NotNull Document document) {
		...
		for (XmlAttribute attrib : PsiTreeUtil.findChildrenOfType(astNode.getPsi(), XmlAttribute.class)) {
			XmlAttributeValue value = attrib.getValueElement();
			if (value == null) {
				continue;
			}
